...
SourceSet hsc::meas::fit::readCat(const char* fname) {
    fitsfile *fptr;
...
std::vector<SourceMatch> hsc::meas::fit::readMatchList(const char* fname) {
    fitsfile *fptr;
    int status = 0, hdutype;
    int colnum;
    long nrows;
...
double *sipfit(int order,
	       SourceSet const &img,
	       SourceSet const &cat) {
    int ncoeff = (order + 1) * (order + 2) / 2 - 3;
    int *xorder = new int[ncoeff];
    int *yorder = new int[ncoeff];
...
hsc::meas::fit::fitTANSIP(int order,
	...
	for (int i = 0; i < npair; i++) {
	    ...
	    x[i] = cos(dec*D2R)*sin((ra-crval[0])*D2R) / (sin(crval[1]*D2R)*sin(dec*D2R)+cos(crval[1]*D2R)*cos(dec*D2R)*cos((ra-crval[0])*D2R)) * R2D;
	    y[i] = (cos(crval[1]*D2R)*sin(dec*D2R)-sin(crval[1]*D2R)*cos(dec*D2R)*cos((ra-crval[0])*D2R)) / (sin(crval[1]*D2R)*sin(dec*D2R)+cos(crval[1]*D2R)*cos(dec*D2R)*cos((ra-crval[0])*D2R)) * R2D;
	    Source::Ptr s1 = Source::Ptr(new Source());
	    s1->setXAstrom(x[i]);
	    s1->setYAstrom(y[i]);
	    cat.push_back(s1);
	    ...
	    s2->setYAstrom(v[i]);
	    img.push_back(s2);
	    //std::cout << ra << " " << dec << " " << x[i] << " " << y[i] << " " << u[i] << " " << v[i] << std::endl;
	    flag.push_back(1);
	}

	coeff = polyfit2(order, img, cat, flag);
	if (verbose) {
	    for (int i = 0; i < ncoeff; i++) {
		printf("%2d %12.5e %12.5e\n", i, coeff[i], coeff[ncoeff+i]);
	    }
	    printf("\n");
	}
	...
	for (int i = 0; i < npair; i++) {
	    if (flag[i] == 1) {
		transform2(order, coeff, u[i], v[i], &xn, &yn);
		s2 += (xn-x[i])*(xn-x[i]) + (yn-y[i])*(yn-y[i]);
		n++;
	    }
	}
	if (verbose) {
	    std::cout << sqrt(s2/n)*3600. << std::endl;
	}
	//std::cout << crval[0] << " " << crval[1] << std::endl;
	lsst::afw::geom::Point2D offset;
	offset[0] = coeff[0]/cos(crval[1]*D2R);
	offset[1] = coeff[ncoeff];
	crval.shift(lsst::afw::geom::Extent2D(offset));
	//std::cout << crval[0] << " " << crval[1] << std::endl;
    }

    Eigen::Matrix2d cd; cd << coeff[1], coeff[2], coeff[ncoeff+1], coeff[ncoeff+2];
	...
	for (int i = k; i >= 0; i--) {
	    int j = k - i;
	    int n = k*(k+1)/2;
	    sipA(i,j) = ( cd(1,1)*coeff[n+j] - cd(0,1)*coeff[ncoeff+n+j]) / D;
	    sipB(i,j) = (-cd(1,0)*coeff[n+j] + cd(0,0)*coeff[ncoeff+n+j]) / D;
	}
    }
    if (verbose) {
    ...
    if (verbose) {
	for (int i = 0; i < ncoeff-3; i++) {
	    printf("%2d %12.5e %12.5e\n", i, coeff[i], coeff[ncoeff-3+i]);
	}
    }
	...
	for (int i = k; i >= 0; i--) {
	    int j = k - i;
	    int n = k*(k+1)/2;
	    sipAp(i,j) = coeff[n-3+j];
	    sipBp(i,j) = coeff[ncoeff-3+n-3+j];
	}
    }
    ...
    //lsst::afw::image::Wcs wcs(crval, crpix, cd);
...
SourceGroup 
hsc::meas::fit::mergeMat(vvSourceMatch const &matchList) {
    SourceGroup allMat;
...
SourceGroup 
hsc::meas::fit::mergeSource(SourceGroup const &sourceSet, SourceGroup const &allMat, double d_lim,
	      ...
	      WcsDic &wcsDic, bool verbose)
{
    ...
    //int ndim = ncoeff * nexp * 2 + nexp * 2 + nstar * 2;
    int ndim = ncoeff * nexp * 2 + nexp * 2;
    if (verbose) {
	std::cout << ncoeff << " " << nexp << " " << nstar << " " << ndim << " " << std::endl;
    }
	...
	*/
    ...
    //double *solution = new double[ndim];
    double *solution = new double[ndim+nstar*2];
    for (int i = 0; i < ndim; i++) {
	solution[i] = x->data[i];
    }
    for (int i = ndim; i < ndim+nstar*2; i++) {
	solution[i] = 0.0;
    }
...
std::vector<double> hsc::meas::fit::solveMosaic(int order,
						SourceGroup const &allMat, SourceGroup const &allSource,
						WcsDic &wcsDic, bool internal, bool verbose)
{
	...
	*/
    ...
    //std::cout << allMat.size() << std::endl;
    //std::cout << allSource.size() << std::endl;
    /*
    for (unsigned int i = 0; i < allSource.size(); i++) {
	SourceSet ss = allSource[i];
	double sr = 0.0;
	...
	std::cout << ss[0]->getRa() << " " << ss[0]->getDec() << " " << sr/sn << " " << sd/sn <<std::endl;
    }

    std::cout << wcsDic[0]->getPixelOrigin() << std::endl;
    */
    /*
    for (unsigned int i = 0; i < allMat.size(); i++) {
	SourceSet ss = allMat[i];
	...
	for (unsigned int j = 1; j < ss.size(); j++) {
	    int iexp = ss[j]->getAmpExposureId();
	    lsst::afw::geom::PointD crval = wcsDic[iexp]->getSkyOrigin();
	    crval[0] = crval[0]  * M_PI / 180.;
	    crval[1] = crval[1]  * M_PI / 180.;
	    lsst::afw::geom::PointD crpix = wcsDic[iexp]->getPixelOrigin();
	    double xi  = calXi(ra, dec, crval[0], crval[1]);
	    ...
	    double xi1 = 0.0;
	    for (int k = 0; k < ncoeff; k++) {
		xi1 += x->data[ncoeff*2*iexp+k] * pow(u, xorder[k]) * pow(v, yorder[k]);
	    }
	    std::cout << xi << " " << xi1 << " " << xi1-xi << std::endl;
	}
    }
    */
    /*
	...
	for (unsigned int j = 1; j < ss.size(); j++) {
	    ...
	    double da = x->data[ndim0+istar*2];
	    double dd = x->data[ndim0+istar*2+1];

	    double xi0 = xi + xi_a * da + xi_d * dd;
	    double xi1 = 0.0;
	    for (int k = 0; k < ncoeff; k++) {
		xi1 += x->data[ncoeff*2*iexp+k] * pow(u, xorder[k]) * pow(v, yorder[k]);
	    }
	    std::cout << xi << " " << xi0 << " " << xi1 << " " << xi0-xi1 << std::endl;
	}
	istar++;
    }
    */
